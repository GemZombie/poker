...
function RunHand(numPlayers) {
    const deck = new POKER.Deck();
    deck.shuffle();
    const board = deck.deal(5);
    const allStartingHands = GetPlayerHands(numPlayers, deck);
    return GetTableBestHand(allStartingHands, board);
}

function GetTableBestHand(players, board) {
    const allBestHands = [];
    for (let pindex = 0; pindex < players.length; pindex++) {
        allBestHands.push(GetPlayerBestHand(players[pindex], board));
    }
    return POKER.getWinners(allBestHands)[0];
}

function GetPlayerHands(numHands, deck) {
    const playerHands = [];
    for (let i = 0; i < numHands; i++) {
        playerHands.push(deck.deal(4));
    }
    return playerHands;
}

function GetPlayerBestHand(playerHand, omahaBoard)  {
    const combos = GetTwoCardCombos(playerHand);
    const boardCombos = GetThreeCardCombos(omahaBoard);
    const fivecardcombos = GetFiveCardHands(combos, boardCombos);
    return POKER.getWinners(fivecardcombos)[0]; // There really can be only one winning hand even if two people have the same hand
}

function GetThreeCardCombos(board) {
    const combos = []
    for (let c1 = 0; c1 < board.length; c1++) {
        for (let c2 = c1+1; c2 < board.length; c2++) {
            for (let c3 = c2 + 1; c3 < board.length; c3++) {
                const combo = [];
                combo.push(board[c1], board[c2], board[c3]);
                combos.push(combo);
            }
        }
...
function GetFiveCardHands(twoCardCombos, threeCardCombos) {
    const hands = [];
    for (c1 = 0; c1 < threeCardCombos.length;c1++) {
        for (c2 = 0; c2 < twoCardCombos.length; c2++) {
            const str = `${threeCardCombos[c1][0]} ${threeCardCombos[c1][1]} ${threeCardCombos[c1][2]} ${twoCardCombos[c2][0]} ${twoCardCombos[c2][1]}`;
            hands.push(POKER.handFromString(str));
        }
    }
    return hands;
}

function GetTwoCardCombos(omahaHand) {
    const combos = [];
    for (c1 = 0; c1 < omahaHand.length; c1++) {
        for (c2 = c1+1; c2 < omahaHand.length;c2++) {
            combos.push([omahaHand[c1], omahaHand[c2]]);
        }
    }
    return combos;
}

function RunSimulation(numHands, numPlayers) {

    for (let i = 0; i < numHands; i++) {
        let hand = RunHand(numPlayers);
        let details = hand.getHandDetails();
        let rank = details.value[0]
        ...
        if (rank == 1) { stats.highCards++; }
        else if (rank == 2) { stats.pairs++ }
        else if (rank == 3) { stats.twoPairs++; }
        else if (rank == 4) { stats.trips++; }
        else if (rank == 5) { stats.straights++; }
        else if (rank == 6) { stats.flushes++; }
        else if (rank == 7) { stats.fullHouses++; }
        else if (rank == 8) { stats.quads++; }
        else if (rank == 9) {
            // The hands come back ordered, so if it's a straight flush and card 1 is an Ace (14) and card 5 is a 10 (10), then it's a royal
            if (val[1] == 14 && val[5] == 10) { stats.royals++; }
            else { stats.straightFlushes++; }
        }
    }
    process.stdout.write('\n');
    process.stdout.write('\n');
    console.log('****Winning Hand Simulations****')
    console.log('** Completed Runs:', numHands);
    console.log('*  Players per Hand: ', numPlayers);
    console.log(' High Cards:       ', stats.highCards.toString().padStart(5), `(${(stats.highCards / numHands * 100).toFixed(2)}%)`);
    console.log(' Pairs:            ', stats.pairs.toString().padStart(5), `(${(stats.pairs / numHands * 100).toFixed(2)}%)`);
    console.log(' Two Pairs:        ', stats.twoPairs.toString().padStart(5), `(${(stats.twoPairs / numHands * 100).toFixed(2)}%)`);
    console.log(' Trips:            ', stats.trips.toString().padStart(5), `(${(stats.trips / numHands * 100).toFixed(2)}%)`);
    console.log(' Straights:        ', stats.straights.toString().padStart(5), `(${(stats.straights / numHands * 100).toFixed(2)}%)`);
    console.log(' Flushes:          ', stats.flushes.toString().padStart(5), `(${(stats.flushes / numHands * 100).toFixed(2)}%)`);
    console.log(' FullHouses:       ', stats.fullHouses.toString().padStart(5), `(${(stats.fullHouses / numHands * 100).toFixed(2)}%)`);
    console.log(' Quads:            ', stats.quads.toString().padStart(5), `(${(stats.quads / numHands * 100).toFixed(2)}%)`);
    console.log(' Straight Flushes: ', stats.straightFlushes.toString().padStart(5), `(${(stats.straightFlushes / numHands * 100).toFixed(2)}%)`);
    console.log(' Royals Flushes:   ', stats.royals.toString().padStart(5), `(${(stats.royals / numHands * 100).toFixed(2)}%)`);
    console.log('********************************');
}

RunSimulation(NUM_HANDS, NUM_PLAYERS);
